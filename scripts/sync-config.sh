...
base = {
    ...
    "AzureSearch": {
        ...
        "IndexName": cfg["azureSearch"].get("indexName", "knowledge-chunks"),
        "DataSourceName": cfg["azureSearch"].get("dataSourceName", "knowledge-blob-datasource"),
        "SkillsetName": cfg["azureSearch"].get("skillsetName", "knowledge-pdf-skillset"),
        "IndexerName": cfg["azureSearch"].get("indexerName", "knowledge-blob-indexer"),
        "SemanticRankerEnabled": cfg["azureSearch"].get("semanticRankerEnabled", False),
        "SemanticConfigurationName": cfg["azureSearch"].get("semanticConfigurationName", "knowledge-semantic"),
        "ChunkMaxPageLength": cfg["azureSearch"].get("chunkMaxPageLength", 1000),
        "ChunkPageOverlapLength": cfg["azureSearch"].get("chunkPageOverlapLength", 200),
        "IngestionPollTimeoutSeconds": cfg["azureSearch"].get("ingestionPollTimeoutSeconds", 120),
        "AdminApiVersion": cfg["azureSearch"].get("adminApiVersion", "2024-07-01"),
    },
    "AzureOpenAI": {
        ...
        "EmbeddingModelName": cfg["azureOpenAI"].get("embeddingModelName", "text-embedding-3-small"),
        "EmbeddingDimensions": cfg["azureOpenAI"].get("embeddingDimensions", 1536),
        "ChatEnabled": cfg["azureOpenAI"].get("chatEnabled", True),
    },
    "AzureStorage": {
        "ConnectionString": cfg.get("storage", {}).get("connectionString", ""),
        "ContainerName": cfg.get("storage", {}).get("containerName", "knowledge-docs"),
    },
    "Services": {
...
sync_secrets("src/KnowledgeService", {
    ...
    "AzureSearch:IndexName": search.get("indexName", "knowledge-chunks"),
    "AzureSearch:DataSourceName": search.get("dataSourceName", "knowledge-blob-datasource"),
    "AzureSearch:SkillsetName": search.get("skillsetName", "knowledge-pdf-skillset"),
    "AzureSearch:IndexerName": search.get("indexerName", "knowledge-blob-indexer"),
    "AzureSearch:AdminApiVersion": search.get("adminApiVersion", "2024-07-01"),
    "AzureStorage:ConnectionString": storage,
    "AzureStorage:ContainerName": cfg.get("storage", {}).get("containerName", "knowledge-docs"),
    "AzureOpenAI:Enabled": "true",
    "AzureOpenAI:Endpoint": oai["endpoint"],
    "AzureOpenAI:ApiKey": oai["apiKey"],
    "AzureOpenAI:EmbeddingDeployment": oai.get("embeddingDeployment", "text-embedding-3-small"),
    "AzureOpenAI:EmbeddingModelName": oai.get("embeddingModelName", "text-embedding-3-small"),
    "AzureOpenAI:EmbeddingDimensions": str(oai.get("embeddingDimensions", 1536)),
})
