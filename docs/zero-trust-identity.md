...
| **Verify nhanh** | Chạy backend/frontend và gọi endpoint trực tiếp; xem [`scripts/README.md`](../scripts/README.md). Re-index cần user JWT hoặc UI. |
...
| [`scripts/README.md`](../scripts/README.md) | Quy ước không tạo shell scripts; AI agent chạy lệnh trực tiếp |
...
|-------|-------------------------|------|------------|
| **0** | — | IaC Entra: `identity.tf` | **Done** |
| **1** | I — Identity foundation | Terraform + cấu hình trực tiếp appsettings/user-secrets/frontend env; xác nhận apps trên Entra portal | **Done** |
...
1. **Không** thêm script `az ad` song song Terraform — Entra SSOT: `identity.tf`.
...
5. Không có mock auth trong repo — token smoke nếu dùng phải là token thật.
6. Trước khi merge branch identity, chạy kiểm tra endpoint trực tiếp và ghi lại lệnh/kết quả trong review.
...
```bash
# Configure Entra values from Terraform output into appsettings/user-secrets/frontend env directly.
...
cd ../..
# Kiểm tra config/azure.local.json có section "entra"; copy các giá trị cần thiết trực tiếp vào local config.
```
...
Prerequisite: backend services chạy trực tiếp bằng `dotnet run`, frontend `npm run dev`, user login tab **Entra / Login**.
...
| Re-index (KnowledgeAdmin) | **OK qua UI** (tab Knowledge + login); direct smoke cần user JWT |

### Smoke test với Entra bật

```bash
# Run backend services directly, then call the protected endpoints with real Bearer tokens.
# For re-index, use a delegated KnowledgeAdmin JWT from UI or entra-browser-session.local.json.
```
